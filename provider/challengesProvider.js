const { selectCurrentChallenges, selectEndChallenges, insertChallenges } = require("../dao/challengesDao");
...
module.exports.addChallenge = async(title, targetCount, countUnit, endAt, notificationPlan) => {
    const connection = await pool.getConnection(async (conn) => (conn));
    const addChallengeResult = insertChallenges(connection, title, targetCount, countUnit, endAt, notificationPlan);
    connection.release();
